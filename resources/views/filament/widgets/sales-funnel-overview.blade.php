...
<x-filament-widgets::widget>
    <x-filament::section>
        ...
        <x-slot name="description">
            RFQ → quote → accepted → invoice → paid
        </x-slot>
        ...
        <div class="fi-sales-funnel flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-stretch sm:gap-2.5">
            @foreach ($stages as $index => $stage)
                ...
                @if ($index < count($stages) - 1)
                    <div class="fi-sales-funnel-conversion flex flex-row items-center justify-start gap-1.5 px-1 py-0.5 text-left sm:min-w-[3rem] sm:flex-col sm:items-center sm:justify-center sm:px-0 sm:text-center">
